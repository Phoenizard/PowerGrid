"""
Test script for N=67, ENSEMBLE_SIZE=50
Results saved with _N67 suffix to avoid overwriting existing data.
"""
...
config.NP_CROSS = 22  # Scale proportionally: 16/50 * 67 ≈ 21.4 → 22
...
def run_simplex_sweep():
    """Run simplex sweep for Fig. 1C with N=67."""
    print("=" * 60)
    print(f"SIMPLEX SWEEP (N={config.N}, ENSEMBLE={config.ENSEMBLE_SIZE})")
    print("=" * 60)

    output_file = os.path.join(config.OUTPUT_DIR, 'data_simplex_q0.0_N67.npz')
    ...
    print(f"Parameters: N={config.N}, q={q}, realizations={n_realizations}, step={step}")
...
def run_cross_section_sweep():
    """Run cross-section sweep for Fig. 1D with N=67."""
    print("\n" + "=" * 60)
    print(f"CROSS-SECTION SWEEP (N={config.N}, NP_CROSS={config.NP_CROSS})")
    print("=" * 60)

    output_file = os.path.join(config.OUTPUT_DIR, 'data_crosssec_N67.npz')
    os.makedirs(config.OUTPUT_DIR, exist_ok=True)
    ...
    print(f"Parameters: N={config.N}, np={n_passive}, n_minus from {n_minus_range[0]} to {n_minus_range[-1]}")
...
if __name__ == '__main__':
    ...
    print(f"Test Config: N={config.N}, ENSEMBLE={config.ENSEMBLE_SIZE}, STEP={config.STEP_SIZE}")
    print()
    ...
    print("Output files:")
    print("  - output/data_simplex_q0.0_N67.npz")
    print("  - output/data_crosssec_N67.npz")
    print("=" * 60)
